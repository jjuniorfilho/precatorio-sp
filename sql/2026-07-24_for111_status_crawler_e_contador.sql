-- FOR-111 — (1) status do crawler_queue por linha (cnj) e (2) contador exato de resultados.
--
-- IMPORTANTE sobre o contador: buscar_processos_agrupado já lê de mv_processos_agrupado
-- (pré-agregada, ~88k linhas no máximo) e a query atual JÁ precisa materializar/ordenar todo
-- o conjunto filtrado antes do LIMIT (não tem mais o truque de index-scan+early-exit desde a
-- correção da FOR-107) — então contar esse MESMO conjunto não adiciona custo relevante,
-- é seguro embutir direto no retorno.
--
-- buscar_processos_incidente é diferente: ainda lê direto de incidentes (305k+ linhas) e
-- CONTINUA dependendo do truque de index-scan+early-exit (ORDER BY valor_acao + LIMIT) pra
-- ser rápido — embutir um COUNT(*) ali reintroduziria exatamente o problema já corrigido na
-- FOR-104/107 (materializar tudo antes de saber quantos tem). Por isso o contador desse
-- relatório vira uma RPC separada (contar_processos_incidente), chamada de forma assíncrona
-- pelo front, sem bloquear a listagem principal — se ela demorar/travar, a tela mostra "—"
-- em vez de travar a página toda.
--
-- Aplicar no SQL Editor.

-- -------------------------------------------------------------
-- 1) buscar_processos_agrupado — soma total_processos + crawler_status
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- 2) buscar_processos_incidente — só crawler_status (contador vai à parte, ver função 3)
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- 3) contar_processos_incidente — contador exato, RPC separada e assíncrona (ver aviso
-- acima: pode ser lenta/travar em filtros pouco seletivos — o front chama isso à parte,
-- sem bloquear a listagem, e mostra "—" se falhar).
-- -------------------------------------------------------------
