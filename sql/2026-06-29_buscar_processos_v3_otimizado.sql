-- FOR-74 — buscar_processos v3 OTIMIZADO: as subqueries por linha (autor, advogados,
-- oab, último andamento) rodam só nas linhas da PÁGINA (pós-limit), não em todos os
-- incidentes. O count(*) over() roda sobre o `base` (colunas baratas) → total correto
-- e rápido. Sem isso, o grid sem filtro dava statement timeout. Aplicar no SQL Editor.

-- Índice p/ max(data) por incidente (coluna e filtro de andamento).
create index if not exists idx_andamentos_inc_data on public.andamentos (incidente_id, data desc);
...
as $function$
  ...
  select pg.incidente_id, pg.processo_codigo, pg.cnj, pg.numero_incidente, pg.tipo_previsto, pg.numero_depre,
    ...
    from page pg
   order by pg.valor_acao desc nulls last;
$function$;
