-- FOR-107 (novo relatório) — "Processo × Incidente": grid flat (sem colapsável), uma linha
-- por incidente, só com colunas de identificação: Cumprimento de Sentença (CNJ do
-- cumprimento, com fallback pro CNJ do processo raiz quando o incidente não está ligado a
-- um cumprimento específico), Nº do incidente, Nº DEPRE (.0500, quando existir), Parte
-- ativa (autor) e Parte passiva (ente devedor). Reaproveita EXATAMENTE o mesmo WHERE já
-- testado/indexado do buscar_processos (FOR-104) — sem GROUP BY, então não tem o problema
-- de agregação que motivou a materialized view do grid agrupado (FOR-107); o truque de
-- "ORDER BY indexado + LIMIT" já funciona sozinho aqui.
--
