-- FOR-77/74 — buscar_processos: filtro por advogado (p_advogado) tolerante a
-- variação de espaço no nome. Motivo: partes.advogado_nome às vezes tem espaço
-- duplo ou nbsp (não-quebrável) que some na exibição, mas quebra o ILIKE exato →
-- carteira de advogado SEM OAB (filtrada por nome) voltava vazia mesmo existindo.
-- Fix: normaliza whitespace (nbsp→espaço, colapsa múltiplos, trim) dos DOIS lados
-- antes do ILIKE. Idêntica à versão FOR-77 anterior, só a linha p_advogado muda.
