...
-- SQL Editor. CONCURRENTLY evita lock de escrita longo nas tabelas em produção — se o
-- SQL Editor do Lovable rodar dentro de uma transação implícita (CREATE INDEX
-- CONCURRENTLY não pode rodar em transação), rodar cada statement separadamente.

-- Filtro sempre ativo (where p.flag_sp) em toda chamada, mesmo sem filtro nenhum.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_processos_flag_sp
  ON public.processos (flag_sp) WHERE flag_sp;

-- Chave do JOIN incidentes → processos.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_incidentes_processo_id
  ON public.incidentes (processo_id);

-- ORDER BY valor_acao desc nulls last em toda consulta.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_incidentes_valor_acao
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_partes_incidente_papel
  ON public.partes (incidente_id, papel);

-- Filtro por OAB (p_oab) — igualdade exata, index simples já resolve.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_partes_oab_normalizada
