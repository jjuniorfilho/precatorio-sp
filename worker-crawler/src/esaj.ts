...
function baseHeaders(session?: Session): Record<string, string> {
  return {
    ...
    Referer: `${config.esajBase}/open.do?servico=190101`,
...
export async function showByCodigo(processoCodigo: string, foro: string, session: Session): Promise<string> {
  const params = new URLSearchParams({
    "processo.codigo": processoCodigo,
    "processo.foro": foro,
    "localPesquisa.cdLocal": foro,
  });
  return fetchHtml(`${config.esajBase}/show.do?${params}`, session);
}
