...
-- Tabelas de controle + seed de config.
-- Depende de FOR-73 (coleta_config, enqueue_crawler_job, coleta_runs).
-- NOTA (consolidação): a ingestão roda como worker externo na VPS
-- (worker-crawler/src/ingest-djen.ts, agendado via pm2/cron do SO),
-- não mais via edge function + pg_cron/pg_net — por isso o cron
-- `caderno-dje-diario`/net.http_post da versão original da PR #5
-- foi removido daqui (apontava pra uma edge function não deployada).
-- Re-executável.
-- =============================================================
...
-- =============================================================
-- FIM — FOR-70. Agendamento real do worker: pm2/cron na VPS
-- (ver worker-crawler/src/ingest-djen.ts), não pg_cron/pg_net.
-- eproc fica parqueado em eproc_pendentes (issue futura, FOR-78).
-- =============================================================
